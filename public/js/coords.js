...
//         // 검색된 좌표를 변수에 저장합니다
//         var lat = result[0].y.toFixed(3);
//         var lng = result[0].x.toFixed(3);
...
document.getElementById("search-form").addEventListener("submit", function (event) {
  ...
  geocoder.addressSearch(addressInput.value, function (result, status) {
    if (status === kakao.maps.services.Status.OK) {
      // 검색된 좌표를 변수에 저장합니다
      var lat = Math.round(result[0].y * 1000) / 1000;
      var lng = Math.round(result[0].x * 1000) / 1000;
      var coords = new kakao.maps.LatLng(lat, lng);
      alert("입력한 주소의 좌표는 " + lat + ", " + lng + " 입니다.");
